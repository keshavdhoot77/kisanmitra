...
const FarmerDashboard = ({ user = { name: 'Farmer' } }) => {
  ...
  const quickActions = [
    { icon: <Plus size={32} />, label: 'dashboard.createListing', desc: 'dashboard.createListingDesc', link: '/create-listing', color: 'bg-primary-50 text-primary-600 border-primary-200 hover:bg-primary-100' },
    { icon: <ShoppingCart size={32} />, label: 'dashboard.myOrders', desc: 'dashboard.myOrdersDesc', link: '/orders', color: 'bg-accent-50 text-accent-600 border-accent-200 hover:bg-accent-100' },
    { icon: <Search size={32} />, label: 'dashboard.browseMarket', desc: 'dashboard.browseMarketDesc', link: '/marketplace', color: 'bg-earth-100 text-earth-800 border-earth-200 hover:bg-earth-200' },
    { icon: <MessageCircle size={32} />, label: 'dashboard.messages', desc: 'dashboard.messagesDesc', link: '/messages', color: 'bg-blue-50 text-blue-600 border-blue-200 hover:bg-blue-100' },
  ];
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 flex items-center gap-3">
            {t(`dashboard.good${greetingKey}`)}, {user.name} 👋
          </h1>
          <p className="text-gray-500 mt-2 text-lg">{t('dashboard.welcomeBack')}</p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {quickActions.map((action, idx) => (
            <Link key={idx} to={action.link} className={`p-6 rounded-2xl border-2 transition-all duration-200 flex flex-col items-start gap-4 hover:scale-105 shadow-sm ${action.color}`}>
              ...
              <div>
                <h3 className="font-bold text-lg mb-1">{t(action.label)}</h3>
                <p className="text-sm opacity-80 font-medium">{t(action.desc)}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-gray-900">{t('dashboard.activeListings')}</h2>
            <Link to="/my-listings" className="text-primary-600 font-semibold hover:text-primary-700">{t('common.viewAll')}</Link>
          </div>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <h2 className="text-xl font-bold text-gray-900 mb-6">{t('dashboard.yourStats')}</h2>
          <div className="space-y-6">
            <div>
              <div className="text-sm font-medium text-gray-500 mb-1">{t('dashboard.totalListings')}</div>
              <div className="text-3xl font-bold text-gray-900">12</div>
            </div>
            <div className="w-full h-px bg-gray-100"></div>
            <div>
              <div className="text-sm font-medium text-gray-500 mb-1">{t('dashboard.successfulSales')}</div>
              <div className="text-3xl font-bold text-primary-600">8</div>
            </div>
            <div className="w-full h-px bg-gray-100"></div>
            <div>
              <div className="text-sm font-medium text-gray-500 mb-1">{t('dashboard.rating')}</div>
